...
<script type="template" id="ordersTemplate">
    ...
    <tr>
        ...
        <td class="text-center">
            <button type="submit"
                    onclick="confirmDeliver('{{=it[i].orderSn}}')"
                    id="button-filter search-order" class="btn btn-primary"><i class="fa fa-clock-o">
                </i>
                确认发货
            </button>
        </td>
...
<script>
    ...
    function confirmDeliver(orderSn){
        $.ajax({
            url: "${ctx}/orders/confirmDelivere",
            type: "POST",
            data: {
                orderSn: orderSn
            },
